...
@section('contenido')
    ...
    {!!Form::model($control,['method'=>'PATCH','id'=>'frm_ubicar',
    'route'=>['produccion.devoluciones.devolver',$control->id_control]])!!}
    {{Form::token()}}
    ...
    <div class="col-lg-4 col-sm-6 col-md-6 col-xs-12">
        <div class="form-group">
            <label for="codigo_interno">Producto</label>
            <input type="text"
                   readonly
                   id="codigo_interno"
                   name="codigo_interno"
                   value="{{$control->producto->codigo_interno}} - {{$control->lote}}"
                   class="form-control">
        </div>
    </div>
...
@section('scripts')
    ...
    <script>
        ...
        function existe_producto_pendiente() {
            ...
            let producto_lote = "";
            getInsumos().forEach(function (prod) {
                producto_lote = prod.id_producto + "-" + prod.lote;
                cantidad = cantidades
                    .filter(x => x.className == producto_lote)
                    .map(e => parseFloat(e.value))
                    .reduce((x, y) => x + y, 0);
                if (parseFloat(prod.devolucion) - cantidad > 0) {
        ...
        function buscar_producto(input) {
            ...
            let producto = getInsumos()
                .filter(e => e.producto.codigo_barras == codigo_barras)
                .find(e => e.lote == lote);
            ...
            if (typeof producto != 'undefined') {
                ...
                document.getElementById('ubicacion').focus();
                gl_cantidad_disponible = parseFloat(producto.devolucion);
        ...
        function getInsumos() {
            let insumos = [];
            insumos = @json($detalle_insumos);
            return insumos.filter(x => x.devolucion > 0);
        }
        ...
        function add() {
            ...
            let cantidad = parseFloat(document.getElementById('cantidad').value.trim() == ""
                ? 0 : document.getElementById('cantidad').value.trim());
            let descripcion_producto = document.getElementById('descripcion').value.trim();

            let cantidad_agregada =
                Array.prototype.slice.call
                (document.getElementsByClassName(gl_id_producto + "-" + lote))
                    .map(e => e.value).reduce((x, y) => parseFloat(x) + parseFloat(y), 0);
