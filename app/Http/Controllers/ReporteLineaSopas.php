...
class ReporteLineaSopas extends Controller
{
    ...
    public function reporte_peso_pasta($id)
    {

        $reporte_encabezado = new Reportes();


        $peso = PesoPastaSopasEnc::where('id_peso_pasta_enc', $id)
            ->select(
                'productos.descripcion as PRODUCTO',
                'presentaciones.descripcion as PRESENTACION',
                'peso_pasta_enc.lote as LOTE',
                DB::raw("date_format(peso_pasta_enc.fecha_hora,'%d/%m/%Y %h:%i:%s') as FECHA"),
                'users.nombre as RESPONSABLE'
            )
        ...
        $reporte_encabezado->setTitle('REGISTRO DE PESO DE PASTA  SOPAS INSTANTANEAS')
            ->setCreatedAt(Carbon::now())
            ->setSubtitle('REGISTRO DE PESO DE PASTA  SOPAS INSTANTANEAS')
        ...
        $peso_detalle = PesoPastaSopasDet::where('id_peso_pasta_enc', $id)
            ...
            ->join('users', 'users.id', '=', 'peso_pasta_det.id_usuario')
            ->get();
        ...
        $reporte_detalle = $reporte_encabezado->mapers(
            [
                ...
                'details' => [
                    'MUESTRA' => $peso_detalle
                ]
            ]
        );
        $reporte_encabezado->setHeader($reporte_detalle['headers']->first());
        $view = \View::make('reportes.sopas.peso_pasta',
            [
                'reporte_encabezado' => $reporte_encabezado,
                'reporte_detalle' => $reporte_detalle
            ]
